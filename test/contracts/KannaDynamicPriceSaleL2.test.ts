...
import { KannaToken, KannaDynamicPriceSaleL2 } from "../../typechain-types";
import { SignerWithAddress } from "@nomiclabs/hardhat-ethers/signers";
import {
  getKnnToken,
  getKnnTreasurer,
  getKnnDynamicPriceSaleL2,
  getKnnTokenMock,
  getKnnDynamicPriceSaleL2Factory,
...
const maticPriceInUSD = 10;
const knnPriceInMATIC = knnPriceInUSD / maticPriceInUSD;

const knnPriceInUSDBigNumber = ethers.utils.parseEther(knnPriceInUSD.toString());
const maticPriceInUSDBigNumber = ethers.utils.parseEther(maticPriceInUSD.toString());
const knnPriceInMATICBigNumber = ethers.utils.parseEther(knnPriceInMATIC.toString());

const CLAIM_TYPEHASH = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("Claim(address recipient,uint256 amountInKNN,uint256 ref,uint256 nonce,uint256 chainId)"));
const BUY_TYPEHASH = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("BuyTokens(address recipient, uint256 knnPriceInUSD, uint16 incrementalNonce, uint256 dueDate, uint256 amountInETH, uint256 amountInKNN, uint256 nonce, uint256 chainId)"));

const calculateKnn = (eth: BigNumber) => {
  const amountInUsd = eth.mul(maticPriceInUSDBigNumber);
...
const claimHash = (address: string, amount: number, claimRef: string) => {
  ...
  const messageHash = ethers.utils.keccak256(
    ethers.utils.defaultAbiCoder.encode(
      ...
      [CLAIM_TYPEHASH, address, amount, claimRef, nonce, ethers.provider.network.chainId]
    )
  );
...
const buyHash = (address: string, knnPriceInUSD: BigNumberish, incrementalNonce: BigNumberish, dueDate: BigNumberish, amountInETH: BigNumberish, amountInKNN: BigNumberish) => {
  ...
  const messageHash = ethers.utils.keccak256(
    ethers.utils.defaultAbiCoder.encode(
      ['bytes32', 'address', 'uint256', 'uint16', 'uint256', 'uint256', 'uint256', 'uint256', 'uint256'],
      [BUY_TYPEHASH, address, knnPriceInUSD, incrementalNonce, dueDate, amountInETH, amountInKNN, nonce, ethers.provider.network.chainId]
    )
  );
...
describe("KNN Dynamic Price Sale L2", () => {
  let signers: SignerWithAddress[];
  let knnToken: KannaToken;
  let knnTreasurer: KannaToken | undefined;
  let knnSale: KannaDynamicPriceSaleL2;
  ...
  const deployContracts = async () => {
    ...
    knnSale = await getKnnDynamicPriceSaleL2(deployerWallet, knnToken, knnTreasurer);
  };
  ...
  const getManagerSession = async (): Promise<
    [SignerWithAddress, KannaDynamicPriceSaleL2]
  > => {
  ...
  describe("Setup", async () => {
    ...
    it("should not initialize KNN Sale with invalid token address", async () => {
      ...
      const knnSaleFactory = await getKnnDynamicPriceSaleL2Factory(deployerWallet);
    ...
    describe("should not buy KNN tokens", async () => {
      ...
      it("when amount is greater than available supply", async () => {
        ...
        const eth = balance.add(1).mul(knnPriceInMATICBigNumber);
      ...
      it("when KNN transfer fail", async () => {
        ...
        const knnSaleWithMock = await getKnnDynamicPriceSaleL2(
          deployerWallet,
          mockToken,
          knnTreasurer
        );
