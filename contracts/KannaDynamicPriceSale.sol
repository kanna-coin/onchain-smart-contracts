...
contract KannaDynamicPriceSale is Ownable, AccessControl {
    ...
    function buyTokens(
        ...
        bytes32 signedMessage = ECDSA.toEthSignedMessageHash(
            keccak256(abi.encode(_BUY_TYPEHASH, recipient, knnPriceInUSD, incrementalNonce, dueDate, nonce))
        );
    ...
    function getNonceAndDueDate(address recipient, uint256 seconds) external view returns (uint256, uint256) {
        return (incrementalNonces[msg.sender] + 1, block.timestamp + seconds);
    }
}
